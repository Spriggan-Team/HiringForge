import { Cn as Selection$1, Ct as isAtEndOfNode, Dn as Fragment$1, Mt as isNodeActive, Pt as isNodeSelection, Sn as PluginKey, Ut as markInputRule, Wt as markPasteRule, Xt as nodeInputRule, at as getNodeType, bn as NodeSelection$1, c as Mark$1, en as parseIndentedBlocks, gn as wrappingInputRule, i as Extension, in as renderNestedMarkdownContent, mn as textblockTypeInputRule, ot as getRenderedAttributes, qt as mergeAttributes, rt as getNodeAtPosition, u as Node3, wn as TextSelection$1, wt as isAtStartOfNode, xn as Plugin, y as canInsertNode } from "./dist-CrUeXY7l.js";
import { t as Link } from "./dist-Dw3KrpHq.js";
import { a as UndoRedo, i as TrailingNode, n as Gapcursor, t as Dropcursor } from "./dist-Crnwdcvx.js";
		...
		return {
			...
			"Shift-Tab": ({ editor }) => {
				...
				if (empty) return editor.commands.command(({ tr }) => {
					...
					if (pos - lineStartPos <= spacesToRemove) tr.setSelection(TextSelection$1.create(tr.doc, lineStartPos));
					return true;
				});
				return editor.commands.command(({ tr }) => {
					const { from, to } = selection;
	...
	addProseMirrorPlugins() {
		return [new Plugin({
			...
			props: { handlePaste: (view, event) => {
				...
				if (tr.selection.$from.parent.type !== this.type) tr.setSelection(TextSelection$1.near(tr.doc.resolve(Math.max(0, tr.selection.from - 2))));
		...
		return { setHorizontalRule: () => ({ chain, state }) => {
			...
			return currentChain.command(({ state: chainState, tr, dispatch }) => {
				if (dispatch) {
					...
					if ($to.nodeAfter) if ($to.nodeAfter.isTextblock) tr.setSelection(TextSelection$1.create(tr.doc, $to.pos + 1));
					else if ($to.nodeAfter.isBlock) tr.setSelection(NodeSelection$1.create(tr.doc, $to.pos));
					else tr.setSelection(TextSelection$1.create(tr.doc, $to.pos));
					else {
